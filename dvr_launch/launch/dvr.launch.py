...
def launch_vehicles(context, *args, **kwargs):
    ...
    common_param_file = os.path.join(
        get_package_share_directory("dvr_launch"), "config/common.param.yaml"
    )
    ...
    vehicle_param_file = os.path.join(
        get_package_share_directory("dvr_launch"), "config/vehicle.param.yaml"
    )
...
def generate_launch_description():
    ...
    #################### Launch Vehicle Node ####################
    num_vehicles_param = DeclareLaunchArgument("number_vehicles", default_value="10")
    #############################################################

    #################### Launch RViz2 ####################
    rviz_cfg_path = os.path.join(
        get_package_share_directory("dvr_launch"), "rviz/dvr.rviz"
    )
    ...
    rviz2 = Node(
        ...
        condition=IfCondition(launch.substitutions.LaunchConfiguration("with_rviz")),
        remappings=[
